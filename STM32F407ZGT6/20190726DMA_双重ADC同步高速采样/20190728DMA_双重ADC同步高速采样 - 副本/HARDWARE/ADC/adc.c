...
{    
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化  
    ...
    DMA_InitStructure.DMA_BufferSize = SENDBUFF_SIZE;         //*             //传输数据个数，单位由以下确定，大小要配合定义的数组类型和外设数据类型
  ...
  ADC_CommonInitStructure.ADC_Prescaler = ADC_Prescaler_Div4;//预分频2分频。ADCCLK=PCLK2/2=84/2=42Mhz,ADC时钟最好不要超过36Mhz 
...
{    
        ...
        DMA_SetCurrDataCounter(DMA_CHx,SENDBUFF_SIZE);  
