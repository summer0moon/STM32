...
{    
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化  
    ...
    DMA_InitStructure.DMA_BufferSize = SENDBUFF_SIZE+10;         //*             //传输数据个数，单位由以下确定，大小要配合定义的数组类型和外设数据类型
...
u32	adc_result_IRQ[SENDBUFF_SIZE+10];
...
void DMA2_Stream0_IRQHandler(void)
{
    ...
    {
				for(i=0;i<(SENDBUFF_SIZE+10)/2;i++)//保留前一半的数据
			{
    ...
    if(DMA_GetITStatus(DMA2_Stream0,DMA_IT_TCIF0) != RESET)  //传输完成  
    {
				...
				for(i=(SENDBUFF_SIZE+10)/2;i<(SENDBUFF_SIZE+10);i++)//保留后一半的数据
			{
					adc_result_IRQ[i] = adc_result[i];
			}
			flag = 0;
