...
void Get_Adc_Data()//传入数组的地址
{
	...
	//********************************************************//
		...
		{
					adc1[j*2] = (u16)(adc_result_IRQ[j] & 0x0000ffff)*3.3/4095;
					adc1[j*2+1] = 0;
					adc2[j*2] = (u16)((adc_result_IRQ[j] & 0xffff0000)>>16)*3.3/4095;
					adc2[j*2+1] = 0;
		}	
		
		arm_max_f32(adc1,SENDBUFF_SIZE*2,&adc1_max,&place);//取出ADC1的最大值
		
		arm_max_f32(adc2,SENDBUFF_SIZE*2,&adc2_max,&place);//取出ADC2的最大值
		
				for(j=0;j<SENDBUFF_SIZE;j++)//将数据保存下来放入* adc1和* adc2
		{
					adc1[j*2] = adc1[j*2] - adc1_max/2.0f;
					adc2[j*2] = adc2[j*2] - adc2_max/2.0f;
		}	
		
		
//		for(j=0;j<SENDBUFF_SIZE;j++)
//		{
//			PID_send_data(adc1[j*2],0xA1);//上位机调试
//			PID_send_data(adc2[j*2],0xA2);//上位机调试
//			delay_ms(5);
//		}
